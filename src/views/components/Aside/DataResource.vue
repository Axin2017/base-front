    ...
    <div class="tree-content">
      <el-collapse v-model="activeNames">
        <el-collapse-item :title="item.title" :name="item.id" v-for="(item,index) in treeDataList" :key="index">
          <el-tree :data="item.children" :show-checkbox="true" :props="defaultProps" @check="onNodeCheck" node-key="_element_key" :filter-node-method="filterNode" ref="tree" />
        </el-collapse-item>
      </el-collapse>
    </div>
  </ScrollDiv>
...
import { getServiceResourceTreeData } from '@/api/dataResource'
...
export default {
  ...
  methods: {
    // 获取后台数据展示树
    getMenuData() {
      getServiceResourceTreeData().then(res => {
        if (res.ReturnCode === 0) {
          const treeDataList = this.dataToTree(res.Data)
          const dataGovernaceData = treeDataList.find(treeData => treeData.title === '中山数据治理')
          dataGovernaceData && dataGovernaceData.children && (this.treeDataList = dataGovernaceData.children)
        }
      })
    },
    getNewKeyId() {
    ...
    onNodeCheck(data, node) {
      ...
      if (layersList.length) {
        this.$store.commit('home/setLayerOperate', { type: checked ? 'add' : 'remove', layers: layersList })
      }
    },
    // 树过滤方法
  ...
  computed: {
    layerOperate() {
      // TODO 这里这么写有问题，不应该固定state的模块
      return this.$store.state.home.layerOperate
    },
    userAK() {
      // return this.$store.state.global.userinfo.userKey
      return '7ad9a013ca6c454584d49d3e5231e00f'
    }
  },
  watch: {
    layerOperate(val) {
      ...
      if (val.type === 'add') {
        for (let k = 0; k < val.layers.length; k++) {
          ...
          this.excuteMapMethod('addWebTileLayer', resourceid, areacode, _layerType === 1 ? 'all' : sindex, this.userAK)
        }
      }
      if (val.type === 'remove') {
        for (let k = 0; k < val.layers.length; k++) {
          const { resourceid, areacode, _layerType, sindex } = val.layers[k] // es6 解构
          let id = resourceid + areacode + this.userAK
          this.excuteMapMethod('removeWebTileLayer', id)
        }
      }
    }
  }
